...
public class GameUpdate{
    ...
    private final HashMap<Point2D, Enemy> enemyGrid = new HashMap<>();
    ...
    private boolean bulletkillCheck(Bullet bullet){
        Enemy enemy;
        if( (enemy = enemyGrid.get(new Point2D(bullet.getGridPosX(), bullet.getGridPosY()))) != null ) {
            if (bullet.getBoundsInLocal().intersects(enemy.getBoundsInLocal())) {
                ...
                enemyGrid.remove(new Point2D(enemy.getGridPosX(), enemy.getGridPosY()));
                return true;
            }
        }
        return false;
        ...
        Enemy enemy;
            for (int y = Main.TILES_Y-1; y >= 0 ; y--) {
                for (int x = Main.TILES_X-1; x >= 0; x--) {
                    if( (enemy = enemyGrid.get(new Point2D(x,y))) != null) {
        ...
        for (int i = 0; i < Main.TILES_X; i++) {
            switch (random.nextInt(4)) {
                ...
                case 0, 1, 2 -> {
                    ...
                    enemyGrid.put(new Point2D(i, 0), peon);
                ...
                case 3 -> {
                    ...
                    enemyGrid.put(new Point2D(i, 0), chad);
